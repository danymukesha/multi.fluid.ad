...
generate_sample_summary <- function(data, biofluid_name) {
  ...
  disease_summary <- data %>%
    ...
    mutate(
      ...
      age_summary = sprintf("%.1f ± %.1f", age_mean, age_sd)
    )
...
generate_final_report <- function() {
  report <- paste0(
    ...
    "   - Urine accuracy: ", sprintf("%.2f ± %.2f", urine_cv$summary$mean_accuracy, urine_cv$summary$sd_accuracy), "\n",
    "   - Serum accuracy: ", sprintf("%.2f ± %.2f", serum_cv$summary$mean_accuracy, serum_cv$summary$sd_accuracy), "\n\n",
